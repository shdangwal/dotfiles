import{k as a,o}from"./chunk-BHCSV6LT.js";import l from"./tabgroup-updator-BSQZAEDQ.css"with{type:"css"};var r=class extends HTMLElement{titleInput;constructor(t,e){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=[l],this.tabGroup=t,this.tabIds=e}async updateGroup(){try{let t=this.tabGroup?.id??await chrome.tabs.group({tabIds:this.tabIds,createProperties:{windowId:globalThis.windowId}});await chrome.tabGroups.update(t,{collapsed:!1,title:this.titleInput.value,color:this.color}),this.remove(),$("marked-action")?.remove()}catch(t){console.error(t)}}onColorPick({target:t}){this.color=t.value}render(){let t=e=>o`<label style="--clr:${e}">
