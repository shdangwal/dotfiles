import{f as r,i as a,j as h,n as p,o as n,p as m}from"./chunk-BHCSV6LT.js";var d=class extends HTMLElement{constructor(){super()}onWindowSwitch(i){globalThis.windowId=+i.target.value,fireEvent(document.body,"windowswitch",globalThis.windowId)}onWindowClick({target:i}){if(!i.closest("vt-icon"))return;let t=i.closest("li"),e=new s(t.textContent);return $on(e,"update",({detail:o})=>{t.firstElementChild.textContent=o;let w=+t.id;h({[w]:o}),this.crtWindowId===w&&($(".win-title").textContent=o.slice(0,10))}),this.firstElementChild.appendChild(e)}addWindow({id:i}){this.windows.push(i)}removeWindow(i){let t=this.windows.findIndex(e=>e.id===i);this.windows.splice(t,1)}render(i){let t=e=>n`<li id="${e}">
				<label>
					<input type="radio" name="window" value="${e}" hidden />
					<span>${i[e]?.slice(0,12)??e}</span>
				</label>
				<vt-icon ico="edit"></vt-icon>
			</li>`;return n`<window-list-popup
				id="window-list"
				style="left:2em"
			...
			</div>`}connectedCallback(){this.id="update-win-dialog",this.replaceChildren(this.render()),this.showModal(),$on(this,"toggle",i=>i.newState==="closed"&&this.remove())}};customElements.define("update-window-dialog",s,{extends:"dialog"});var l=class extends HTMLElement{constructor(i,t){super(),this.tabIds=i,this.tabGroupId=t}async moveToWindow({target:i}){try{let o={index:-1,windowId:+i.value||(await chrome.windows.create({state:"maximized"})).id};this.tabIds?await chrome.tabs.move(this.tabIds,o):this.tabGroupId&&await r(this.tabGroupId,o),this.remove(),$("marked-action")?.remove()}catch{console.error()}}onWindowClick({target:i}){let t=i.closest("li");if(t.id==="newWindow")return this.moveToWindow({target:i});if(!i.closest("vt-icon"))return;let e=new s(t.textContent);return $on(e,"update",({detail:o})=>t.firstElementChild.textContent=o),this.firstElementChild.appendChild(e)}render(){let i=t=>n`<li>
