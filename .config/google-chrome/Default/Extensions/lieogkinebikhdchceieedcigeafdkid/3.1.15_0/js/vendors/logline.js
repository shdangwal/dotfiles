(function (global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? (module.exports = factory()) : typeof define === "function" && define.amd ? define(factory) : (global.Logline = factory());
})(this, () => {
	"use strict";
	...
	// NOTE: if WechatFE/vConsole is detected, will not use %c feature, as it is not well supported
	function debug(namespace, level, descriptor, data) {
		if (HAS_CONSOLE && config.get().verbose) {
			console[LEVEL_CONSOLE_MAP[level.toUpperCase()] || LEVEL_CONSOLE_MAP.INFO]("[" + namespace + "] " + level.toUpperCase() + " " + descriptor, data || "");
		}
	}
			...
			[
				{
					...
					value: function init(database) {
						...
						try {
							...
							IndexedDBLogger.request = indexedDB.open(IndexedDBLogger._database);
					...
					value: function get$$1(from, to, readyFn) {
						try {
							if (IndexedDBLogger.status !== get$1(IndexedDBLogger.__proto__ || Object.getPrototypeOf(IndexedDBLogger), "STATUS", this).INITED) {
								return IndexedDBLogger._pool.push(() => {
				...
				},
				{
					...
					value: function clean() {
						try {
							...
							var request = indexedDB.deleteDatabase(IndexedDBLogger._database);
							request.onerror = function (event) {
								return throwError(event.target.error);
							};
				...
				},
				{
					key: "support",
					get: function get$$1() {
						var support = !!(indexedDB && IDBTransaction && IDBKeyRange);
						return support;
					},
				},
			]);
			...
			[
				{
					...
					value: function _record(level, descriptor, data) {
						var logs;
						try {
							logs = localStorage.getItem(LocalStorageLogger._database) ? JSON.parse(localStorage.getItem(LocalStorageLogger._database)) : [];
							logs.push([Date.now(),
								this._namespace,
								level,
								descriptor,
								data]);
							debug(this._namespace, level, descriptor, data);
							localStorage.setItem(LocalStorageLogger._database, JSON.stringify(logs));
						} catch (e) {
							localStorage.removeItem(LocalStorageLogger._database);
							localStorage.setItem(LocalStorageLogger._database, JSON.stringify([]));
			...
			[
				{
					...
					value: function init(database) {
						try {
							...
							if (!localStorage.getItem(LocalStorageLogger._database)) {
								localStorage.setItem(LocalStorageLogger._database, JSON.stringify([]));
							}
				...
				},
				{
					...
					value: function get$$1(from, to, readyFn) {
						var logs, i;
						try {
							logs = JSON.parse(localStorage.getItem(LocalStorageLogger._database));
				...
				},
				{
					...
					value: function keep(daysToMaintain) {
						...
						try {
							logs = !daysToMaintain
								? []
								: (localStorage.getItem(LocalStorageLogger._database) ? JSON.parse(localStorage.getItem(LocalStorageLogger._database)) : []).filter((log) => {
					...
					value: function _record(level, descriptor, data) {
						...
						try {
							...
							WebsqlLogger._db.transaction((tx) => {
								tx.executeSql("INSERT INTO logs (time, namespace, level, descriptor, data) VALUES(?, ?, ?, ? ,?)",
									[Date.now(),
										_this2._namespace,
										level,
										descriptor,
			...
			[
				{
					...
					value: function init(database) {
						...
						try {
							WebsqlLogger._db = openDatabase(WebsqlLogger._database, "1.0", "cats loves logs", 4.85 * 1024 * 1024);
							WebsqlLogger._db.transaction((tx) => {
								tx.executeSql("CREATE TABLE IF NOT EXISTS logs (time, namespace, level, descriptor, data)",
									[],
									() => {
				...
				},
				{
					...
					value: function get$$1(from, to, readyFn) {
						if (WebsqlLogger.status !== get$1(WebsqlLogger.__proto__ || Object.getPrototypeOf(WebsqlLogger), "STATUS", this).INITED) {
							return WebsqlLogger._pool.push(() => {
						...
						try {
							WebsqlLogger._db.transaction((tx) => {
								tx.executeSql("SELECT * FROM logs ORDER BY time DESC",
									[],
									(tx, res) => {
										var logs = [],
											line,
				...
				},
				{
					...
					value: function keep(daysToMaintain) {
						...
						try {
							WebsqlLogger._db.transaction((tx) => {
								if (daysToMaintain) {
									tx.executeSql("DELETE FROM logs WHERE time < ?",
